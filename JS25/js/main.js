...
let app = new Vue({
    ...
    computed: {
        filterGoods() {
            return this.product.filter(item => item.product_name.includes(this.searchLine.value) !== -1)
        }
    },
    methods: {
        getJson(url){
        ...
        addProduct(product){
            ...
            this.CartProduct = product.product_name;
            return [this.CartProduct === this.CartProduct, this.CartPrice === this.CartPrice ];
        }
    },
    mounted(){
